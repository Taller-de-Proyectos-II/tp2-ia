...
class MultiLayer:
    ...
    def __init__(self, input):
        ...
        with open('weights_wi.csv', newline = '') as File:  
            rows = csv.reader(File)
            for row in rows:
                self.wi.append([float(x) for x in row])
            

        with open('weights_wh.csv', newline = '') as File:  
            rows = csv.reader(File)
            for row in rows:
    ...
    def training(self, input, outputs, iterations = 10000, error = 0.0001):
        learn = False
        iter = 0
        average = 0
        ...
        while not learn:
            ...
            if error >= average or iter >= iterations:
                """
                with open('weights_wh.csv', 'w', newline = '') as File:
                    writer = csv.writer(File)
                    writer.writerows(self.wh)

                with open('weights_wi.csv', 'w', newline = '') as File:
                    writer = csv.writer(File)
